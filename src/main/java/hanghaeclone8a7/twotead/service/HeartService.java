...
public class HeartService {
    ...
    public ResponseDto<?> heart(Long id, UserDetails userDetails, HttpServletRequest request){
        ...
        if(post.isPresent()) {
            ...
            System.out.println("postExists = " + postExists);

            // 좋아요 취소
            if (postExists.isPresent()) {
                heartRepository.delete(postExists.get());
                List<Heart> heartList = heartRepository.findByJobPostId(id);
                System.out.println("heartList delete = " + heartList.size());

                HeartResponseDto heartResponseDto = HeartResponseDto.createHeartResponseDto(false, heartList);
            ...
            // 좋아요
            List<Heart> heartList = heartRepository.findByJobPostId(id);
            System.out.println("heartList save = " + heartList.size());
            HeartResponseDto heartResponseDto = HeartResponseDto.createHeartResponseDto(true, heartList);
